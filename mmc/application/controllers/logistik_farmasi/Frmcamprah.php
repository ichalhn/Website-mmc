...
class Frmcamprah extends Secure_area
{
  public function __construct(){
    ...
    function get_distribusi_from_amprah_list(){
        //echo sizeof($_POST);
        $line  = array();
        $line2 = array();
        $row2  = array();
        if(sizeof($_POST)==0) {
            ...
            $hasil = $this->Frmmamprah->getdata_distribusi_from_amprah($this->input->post());
            /*$line['data'] = $hasil;*/
            foreach ($hasil as $value) {
                $row2['id_amprah'] = $value->id_amprah;
                $row2['tgl_amprah'] = $value->tgl_amprah;
				...
				//Sementara
				$cek = $this->Frmmamprah->getSisaAmprah($value->id_amprah)->row();

                if ($cek->sisa > 0 || $cek->sisa == NULL) {
                    $row2['status'] = '<font color="red">Open</font>';
                }else {
                    $row2['status'] = '<font color="green">Closed</font>';
                }
		...
		*/  
		...
		$konten = $konten ."
		  ...
		  <tr>
					...
					<tr>
					  <td width=\"10%\"></td>
					  <td width=\"35%\">Kepala Instalasi Farmasi</td>
					  <td width=\"18%\"> </td>
					  <td width=\"40%\">Apoteker Koordinator Perbekalan</td>
					</tr>
					<tr>
		  			  <td></td>
					  <td></td>
        ...
        <tr>
          <td width="6%"></td>
          <td width="30%"><b>Meriyanti Permata Sari, S.Farm Apt</b></td>
          <td width="30%"> </td>
          <td width="30%"><b>Rekhe Olivia, S.Farm Apt</b></td>
        </tr>
        <tr>
          <td width="5%"></td>
          <td width="30%">SIPA: 19810531/SIPA.16.03/2016/2010</td>
          <td width="25%"> </td>
          <td width="35%">SIPA: 19811021/SIPA.16.05/2015/2004</td>
        </tr>
        */
