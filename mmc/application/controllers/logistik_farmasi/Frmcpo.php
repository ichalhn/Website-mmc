...
class Frmcpo extends Secure_area
{
  public function __construct(){
	...
	function getNomorPOCari(){
		$cari = $this->input->post('tanggal');

		$data = array();
		$data['data'] = $this->Frmmpo->getNomorPOCari($cari);
   ...
   /*    echo "<pre>";
    ...
    function cari_data_obat_new(){
        if (isset($_GET['term'])){
            $q = strtolower($_GET['term']);
            $this->Frmmpo->autocomplete_obat($q);
            //echo json_encode($q);
        }
    }
    ...
    public function cari_data_obat_baru(){
        $keyword = $this->uri->segment(4);
        $data = $this->db->query("SELECT * FROM master_obat WHERE nm_obat LIKE '%".$keyword."%' LIMIT 20")->result();
        $arr='';
        if(!empty($data)){
            foreach($data as $row)
            {
		...
		$konten = $konten ."
		  ...
		  <table border=\"0\">
		  			...
		  			<tr>
		  			  <td width=\"30%\"></td>
		  			  <td width=\"30%\"></td>
					  <td width=\"40%\" align=\"right\">Palembang, ___ ______________ 20____</td>
					</tr>
                     <tr>
					  <td width=\"30%\">Menyetujui,</td>
					</tr>
					<tr>
					  <td width=\"35%\">Direktur Rs. MMC Palembang</td>
					  <td width=\"15%\"> </td>
					  <td width=\"25%\"> </td>
                    ...
                    <tr>
                     <br/><br/><br/><br/>
					  <td width=\"30%\"><b>(___________________________)</b></td>
					  <td width=\"20%\"> </td>
					  <td width=\"20%\"> </td>
					  <td width=\"30%\"><b>(___________________________)</b></td>
					</tr>
