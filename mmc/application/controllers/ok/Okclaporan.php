...
class Okclaporan extends Secure_area {
	...
	public function data_kunjungan2()
	{
	...
	{
		$data['title'] = 'Laporan Kunjungan Penunjang Kamar Operasi ';				
		...
		$data['message_nodata']="<div class=\"content-header\">
			<div class=\"alert alert-danger alert-dismissable\">
				<button class=\"close\" aria-hidden=\"true\" data-dismiss=\"alert\" type=\"button\">×</button>				
			<h4><i class=\"icon fa fa-close\"></i>
				Silahkan Pilih Tanggal dan Download untuk Melihat Laporan Kunjungan.
			</h4>							
			</div>
		</div>";

		$this->load->view('ok/okvkunjungan',$data);
	}
	...
	public function download_keuangan($param1='',$param2=''){
		...
		// Set document properties  
		$objPHPExcel->getProperties()->setCreator("RS Khusus Mata")  
		        ->setLastModifiedBy("RS Khusus Mata")  
		        ->setTitle("Laporan Keuangan RS Khusus Mata")  
		        ->setSubject("Laporan Keuangan RS Khusus Mata Document")  
		        ->setDescription("Laporan Keuangan RS Khusus Mata, generated by HMIS.")  
		        ->setKeywords("RS Khusus Mata")  
		        ->setCategory("Laporan Keuangan");  
		...
		// Rename worksheet (worksheet, not filename)  
		$objPHPExcel->getActiveSheet()->setTitle('RS Khusus Mata');    
	...
	public function download_kunjungan($param1='',$param2=''){
		////EXCEL 
		$this->load->library('Excel');  
		...
		$objPHPExcel->getProperties()->setCreator("RS Khusus Mata")  
		        ->setLastModifiedBy("RS Khusus Mata")  
		        ->setTitle("Laporan Kunjungan RS Khusus Mata")  
		        ->setSubject("Laporan Kunjungan RS Khusus Mata Document")  
		        ->setDescription("Laporan Kunjungan RS Khusus Mata, generated by HMIS.")  
		        ->setKeywords("RS Khusus Mata")  
		        ->setCategory("Laporan Kunjungan");  
		...
		$objPHPExcel=$objReader->load(APPPATH.'third_party/lap_kun_ok.xlsx');
		// Set active sheet index to the first sheet, so Excel opens this as the first sheet  
		$objPHPExcel->setActiveSheetIndex(0);  
		// Add some data  
      	$objPHPExcel->getActiveSheet()->getColumnDimension('A')->setAutoSize(true);
      	...
      	$objPHPExcel->getActiveSheet()->getStyle('C3')->getFont()->setBold(true);
      	$objPHPExcel->getActiveSheet()->getStyle('C3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
      	$objPHPExcel->getActiveSheet()->getStyle('C3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A1', "Laporan Kunjungan Kamar Operasi Periode ".date('d F Y', strtotime($param1))." - ".date('d F Y', strtotime($param2)));
      	$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setBold(true);
      	$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setSize(16);
      	$objPHPExcel->getActiveSheet()->mergeCells('A1:K1');
      	$objPHPExcel->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
      	$objPHPExcel->getActiveSheet()->getStyle('A1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$objPHPExcel->getActiveSheet()->SetCellValue('A2', "RUMAH SAKIT KHUSUS MATA PROVINSI SUMATERA SELATAN");
		...
		$vtot_odc_umum = 0;
		$vtot_odc_bpjs = 0;
		$vtot_odc_jamsoskes = 0;
		$vtot_odc_dijamin = 0;
		$vtot_ri_umum = 0;
		$vtot_ri_bpjs = 0;
		$vtot_ri_jamsoskes = 0;
		$vtot_ri_dijamin = 0;
		$data_kunjungan=$this->okmlaporan->get_data_kun($param1, $param2)->result();
		$data_kunjungan_count=$this->okmlaporan->get_data_kun_count($param1, $param2)->result();
		foreach($data_kunjungan_count as $row){
			$count_tgl[] = array('tgl' => $row->tgl, 'jumlah' => $row->jumlah);
		}
		foreach($data_kunjungan as $row){
			$total_kunjungan++;
			if($temptgl == $row->tgl){
				if($row->no_register=="RJ" AND $row->cara_bayar=="UMUM"){
					$odc_umum = $odc_umum+1;
				}
				if($row->no_register=="RJ" AND $row->cara_bayar=="BPJS"){
					$odc_bpjs = $odc_bpjs+1;
				}
				if($row->no_register=="RJ" AND $row->cara_bayar=="JAMSOSKES"){
					$odc_jamsoskes = $odc_jamsoskes+1;
				}
				if($row->no_register=="RJ" AND $row->cara_bayar=="DIJAMIN"){
					$odc_dijamin = $odc_dijamin+1;
				}
				if($row->no_register=="RI" AND $row->cara_bayar=="UMUM"){
					$ri_umum = $ri_umum+1;
				}
				if($row->no_register=="RI" AND $row->cara_bayar=="BPJS"){
					$ri_bpjs = $ri_bpjs+1;
				}
				if($row->no_register=="RI" AND $row->cara_bayar=="JAMSOSKES"){
					$ri_jamsoskes = $ri_jamsoskes+1;
				}
				if($row->no_register=="RI" AND $row->cara_bayar=="DIJAMIN"){
					$ri_dijamin = $ri_dijamin+1;
				}
				$tot = $tot+1;
				
				if($tot==$count_tgl[$j]['jumlah']){
					$vtot_odc_umum = $vtot_odc_umum + $odc_umum;
					$vtot_odc_bpjs = $vtot_odc_bpjs + $odc_bpjs;
					$vtot_odc_jamsoskes = $vtot_odc_jamsoskes + $odc_jamsoskes;
					$vtot_odc_dijamin = $vtot_odc_dijamin + $odc_dijamin;
					$vtot_ri_umum = $vtot_ri_umum + $ri_umum;
					$vtot_ri_bpjs = $vtot_ri_bpjs + $ri_bpjs;
					$vtot_ri_jamsoskes = $vtot_ri_jamsoskes + $ri_jamsoskes;
					$vtot_ri_dijamin = $vtot_ri_dijamin + $ri_dijamin;
					$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $i);
					$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $row->tgl);
					$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $odc_umum);
					$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $odc_bpjs);
					$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, $odc_jamsoskes);
					$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $odc_dijamin);
					$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $ri_umum);
					$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $ri_bpjs);
					$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, $ri_jamsoskes);
					$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, $ri_dijamin);
					$objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, $tot);
					$i++;
					$j++;
					$rowCount++;
				}
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $vtot_odc_umum);
		$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $vtot_odc_bpjs);
		$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, $vtot_odc_jamsoskes);
		$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $vtot_odc_dijamin);
		$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $vtot_ri_umum);
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $vtot_ri_bpjs);
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, $vtot_ri_jamsoskes);
		$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, $vtot_ri_dijamin);
		$objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, $total_kunjungan);
		$filename = "Laporan Kunjungan Kamar Operasi ".date('d F Y', strtotime($param1))." - ".date('d F Y', strtotime($param2));
		header('Content-Disposition: attachment;filename="'.$filename.'.xls"');  
