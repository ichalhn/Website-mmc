...
class rjckwitansi extends Secure_area{
	...
	*/
	...
	public function edit_biaya_tambahan(){
		$no_register=$this->input->post('no_register');
		$data['vtot_lain']=$this->input->post('edit_tindakan');
		$data['vtot_lab']=$this->input->post('edit_lab');
		$data['vtot_obat']=$this->input->post('edit_obat');
		$data['vtot_rad']=$this->input->post('edit_diag');
		$data['vtot_ok']=$this->input->post('edit_op');
		if($data['vtot_lab'] == ''){
			$data['status_lab']='0';
		}
		else{
		...
		$this->rjmkwitansi->edit_biaya_tambahan($no_register,$data);
		redirect('irj/rjckwitansi/kwitansi_pasien/'.$no_register);
	}
...
//		print_r($data);
	...
	//	print_r($data);
		...
		if($no_register!=''){
			...
			if(($noncover==1 && $data_pasien->cara_bayar=='DIJAMIN') || $data_pasien->cara_bayar=='UMUM'){
	...
	public function cetak_faktur_kw0_kt($no_register='')
	{
		...
		if($no_register!=''){
			...
			//print_r($detail_daful);exit();
			
			if($detail_daful->cara_bayar=='UMUM'){
				$pasien_bayar='TUNAI';
			}else{ $pasien_bayar='KREDIT';}
			// print_r($pasien_bayar);exit();
	...
	//		print_r($detail_bayar);exit();
					...
					<table class=\"table-font-size2\" border=\"0\">
						<tr>
							<td width=\"16%\">
								...
								<td  width=\"60%\" style=\" font-size:9px;\"><font style=\"font-size:12px\">$namars</font><br>$alamatrs $kota_kab $telp
							</td>
							<td width=\"20%\"><font size=\"8\" align=\"right\">$tgl_jam</font></td>						
						</tr>
						<tr><td></td><td colspan=\"2\"><p align=\"right\" style=\"font-size:10px;\">Pembayaran : <u>".$pasien_bayar."</u></p></td></tr>
					</table>	
					<table>	
							<tr>
								<td colspan=\"3\" ><font size=\"10\" align=\"center\"><u><b>KWITANSI RAWAT JALAN 
								No. $no_register</b></u></font></td>
							</tr>		
							<tr>
								<td></td>
							...
							<tr>
								<td width=\"19%\">Sudah Terima Dari</td>
								<td width=\"2%\"> : </td>
								<td width=\"37%\">".strtoupper($penyetor)."</td>
								<td width=\"19%\">Tgl Kunjungan</td>
								<td width=\"2%\"> : </td>
								<td>".date("d-m-Y",strtotime($data_pasien->tgl_kunjungan))."</td>
							</tr>
							<tr>
			...
			$konten=$konten."<table border=\"0\" style=\"padding:2px\">
	...
	public function cetak_faktur_kt($no_register='')
	{
		...
		if($no_register!=''){
			...
			*/
			...
			$konten="<style type=\"text/css\">
					...
					<table>	
							...
							<tr>
								...
								<td width=\"2%\"> : </td>
								<td width=\"37%\">".strtoupper($penyetor)."</td>
								<td width=\"20%\"><b>Tgl Kunjungan</b></td>
								<td width=\"2%\"> : </td>
								<td>".date("d-m-Y",strtotime($data_pasien->tgl_kunjungan))."</td>
							</tr>
							<tr>
			...
			$data_lab=$this->rjmkwitansi->getdata_unpaid_lab_pasien->result();
			$data_pa=$this->rjmkwitansi->getdata_unpaid_pa_pasien->result();
			$data_rad=$this->rjmkwitansi->getdata_unpaid_rad_pasien->result();
			$data_resep=$this->$this->rjmkwitansi->getdata_unpaid_resep_pasien->result();
			$no=1;
			//print_r($data_tindakan);
			$konten=$konten."<table border=\"1\" style=\"padding:2px\">
						<tr>
