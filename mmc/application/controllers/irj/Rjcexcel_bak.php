...
class Rjcexcel extends Secure_area {
	...
	public function excel_lapkunj($tampil_per='', $id_poli='', $var1='',$cara_bayar='')
	{
		...
		if ($tampil_per=="TGL") {
			...
			if ($id_poli=="SEMUA") {
				...
				$data_laporan_kunj=$this->Rjmlaporan->get_data_kunj_poli_harian($tgl,$cara_bayar)->result();
	...
	public function excel_lapkeu($tampil_per='', $id_poli='', $var1='', $status='', $cara_bayar='',$param1='')
	{
		...
		$namars='test';//$this->config->item('namars');
			$kota_kab=$this->config->item('kota');
			$alamat=$this->config->item('alamat');
			$nmsingkat=$this->config->item('namasingkat');
		...
		if ($tampil_per=="TGL") {
			...
			if ($id_poli=="SEMUA") {
				...
				$file_name="xxxxx";//"KEU_POLI_".$tgl1."_".$tgl11.".xlsx";
				$poli=$this->Rjmpencarian->get_poliklinik()->result();
				...
				$file_name="YYYY";//"KEU_POLI_".$id_poli."_$tgl1_$tgl11.xlsx";
			...
			if ($id_poli=="SEMUA") {
				...
				$file_name="vvvv";//"KEU_POLI_$bulan1.xls";
				$poli=$this->Rjmpencarian->get_poliklinik()->result();
				...
				$file_name="ZZZZZ";//"KEU_POLI_".$id_poli."_$bulan1.xls";
			...
			if ($id_poli=="SEMUA") {
				...
				$file_name="RRR";//"KEU_POLI_$tahun.xls";
				$poli=$this->Rjmpencarian->get_poliklinik()->result();
				...
				$file_name="FFF";//"KEU_POLI_".$id_poli."_$tahun.xls";
		...
		$objPHPExcel->getActiveSheet()->setTitle('Keu');
		header('Content-Disposition: attachment;filename="'.$file_name.'"');
		$objPHPExcel->getActiveSheet()->setTitle($namars);
		//ob_end_flush();
		ob_end_clean();
