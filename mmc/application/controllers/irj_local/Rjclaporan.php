...
class Rjclaporan extends Secure_area {
		...
		if($_SERVER['REQUEST_METHOD']=='POST'){
			...
			if ($data['tampil_per'] == "TGL") {
				...
				if ($this->input->post('id_poli')=="SEMUA") {
					$data['id_poli']="SEMUA";
					$data['poli']=$this->Rjmpencarian->get_poliklinik()->result();
					$data['data_laporan_kunj']=$this->Rjmlaporan->get_data_kunj_poli_bulanan(substr($data['bulan'],5,2))->result();
				} else {
					$select_poli = explode("@", $this->input->post('id_poli'));
					$data['id_poli']=$select_poli[0];
					$data['nm_poli']='<b>'.$select_poli[1].'</b>';
					$data['data_laporan_kunj']=$this->Rjmlaporan->get_data_kunj_bulanan(substr($data['bulan'],5,2),$data['id_poli'])->result();
				}
	...
	public function lapkeu()
	{
		...
		$data['data_laporan_keuangan']= $this->Rjmlaporan->cek_data_keu_poli_harian("", "")->result();
	...
	public function cek_lapkeu(){
		$data['title']='Laporan Keuangan Instalasi Rawat Jalan';

		$data['select_poli']=$this->Rjmpencarian->get_poliklinik()->result();

		$tanggal = $this->input->post('tanggal_laporan');
		$tgl = $this->input->post('tgl_awal');//substr($tanggal,0,10);
		$tgl0 = $this->input->post('tgl_akhir');//substr($tanggal, 12,11);

		// $tgl = date_format(date_create($tgl_awal),'Y-m-d');//date($tgl_awal);
		// // $tgl = date('Y-m-d',$temp_tgl);
		// $tgl0 = DateTime::createFromFormat('d/m/Y', date_create($tgl_akhir));//date_format($tgl_akhir);
		// $tgl1 = $tgl0->format('Y-m-d');
		// // $tgl0 = datdate_createe('Y-m-d',$temp_tgl0);


		$data['data_laporan_keuangan']=$this->Rjmlaporan->cek_data_keu_poli_harian($tgl, $tgl0)->result();
		// print_r($data['data_laporan_keuangan']);exit();
		...
		if($_SERVER['REQUEST_METHOD']=='POST'){
			...
			if ($data['tampil_per'] == "TGL") {
				...
				if ($this->input->post('id_poli')=="SEMUA") {
					$data['id_poli']="SEMUA";
					$data['poli']=$this->Rjmpencarian->get_poliklinik()->result();
					$data['data_laporan_keu']=$this->Rjmlaporan->get_data_keu_poli_bulanan(substr($data['bulan'],5,2), $data['status'], $data['cara_bayar'])->result();
				} else {
					$select_poli = explode("@", $this->input->post('id_poli'));
					$data['id_poli']=$select_poli[0];
					$data['nm_poli']='<b>'.$select_poli[1].'</b>';
					$data['data_laporan_keu']=$this->Rjmlaporan->get_data_keu_bulanan(substr($data['bulan'],5,2),$data['id_poli'], $data['status'], $data['cara_bayar'])->result();
				}
