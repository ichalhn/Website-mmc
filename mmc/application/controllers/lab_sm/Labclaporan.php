...
class Labclaporan extends Secure_area {
	...
	public function data_pendapatan($tampil_per='', $param1='')
	{
		...
		$tgl_indo=new Tglindo();
		// if($_SERVER['REQUEST_METHOD']=='POST'){
		// 	$range_tanggal = $this->input->post('tanggal_laporan');
		// 	$range_laporan = explode(" - ", $range_tanggal);
		// 	$awal = explode("/", $range_laporan[0]);
		// 	$akhir = explode("/", $range_laporan[1]);
		// 	$tgl_awal = $awal[2]."/".$awal[1]."/".$awal[0];
		// 	$tgl_akhir = $akhir[2]."/".$akhir[1]."/".$akhir[0];
		// 	$cara_bayar=$this->input->post('jenis_pasien');	
		// 	$data['data_laporan_keu']=$this->Labmlaporan->get_data_keu_tind($tgl_awal, $tgl_akhir)->result();
		// 	$data['date_title']="Laporan Keuangan Laboratorium <b> $range_tanggal</b>";

		// 	$this->load->view('lab/labvpendapatan',$data);
		// }else{			
		// 	$data['data_laporan_keu']=$this->Labmlaporan->get_data_keu_tindakan_today()->result();
		// 	$data['data_keuangan']=$this->Labmlaporan->get_data_keuangan_today()->result();
		...
		// 			<button class=\"close\" aria-hidden=\"true\" data-dismiss=\"alert\" type=\"button\">×</button>				
		// 		<h4><i class=\"icon fa fa-close\"></i>
		// 			Silahkan Pilih Tanggal dan Download untuk Melihat Laporan Keuangan.
		// 		</h4>							
		// 		</div>
		// 	</div>";
