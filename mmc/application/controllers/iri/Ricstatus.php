...
class Ricstatus extends Secure_area {
	...
	public function index($no_ipd='',$param=''){
		...
		// }else{
			...
			$data['list_resep_subtotal'] = $this->rimpasien->get_list_resep_pasien_subtotal($no_ipd,$pasien[0]['noregasal']);
			$data['list_tindakan_ird'] = $this->rimpasien->get_list_tindakan_ird_pasien($pasien[0]['noregasal']);
			$data['poli_irj'] = $this->rimpasien->get_list_poli_rj_pasien($pasien[0]['noregasal']);
		// }
	...
	public function cetak_list_pembayaran_pasien($no_ipd='',$status=''){
		...
		$pasienold=$this->rimtindakan->get_old_pasien($pasien[0]['noregasal']);
		if($pasien[0]['carabayar']=='BPJS' && $pasienold[0]['cara_bayar']=='BPJS'){
			...
			$list_resep = $this->rimpasien->get_list_resep_pasien($no_ipd,$pasien[0]['noregasal']);
			$list_resep_subtotal = $this->rimpasien->get_list_resep_pasien_subtotal($no_ipd,$pasien[0]['noregasal']);
		}else{
			...
			$list_resep_subtotal = $this->rimpasien->get_list_all_resep_pasien_subtotal($no_ipd);
		}
		...
		$vtotresep=0;
		if(($list_resep)){
			$result = $this->string_table_resep($list_resep, $pasien[0]['carabayar'], $list_resep_subtotal);
			$grand_total = $grand_total + $result['subtotal'];
			$konten = $konten."<table class=\"table-isi\" border=\"0\">
			<tr>
				<td colspan=\"6\">Apotik</td>
				<td colspan=\"2\" align=\"right\"> ".number_format($result['subtotal'],0)."</td>
			</tr></table>
			";
		...
		//biaya_administrasi
		$biaya_administrasi= (double) $grand_total * 0.07;
		...
		$biaya_administrasi= ((double) $grand_total - $vtotresep) * 0.07;
		if($biaya_administrasi<=50000){
			$fix_adm=0;//50000;
		}else if($biaya_administrasi>=750000 && $pasien[0]['carabayar']=='UMUM'){
			$fix_adm=0;//750000;
		...
		$konten = $konten."<tr>
			...
			<tr>
				...
				<td align=\"right\"> ".number_format($biaya_administrasi,0)." </td>
			</tr>
			<!--<tr>
		...
		$grand_total = (double) $grand_total + (double) $fix_adm + (double) $jasa_perawat + 0 + (double) $biaya_daftar;
		...
		$jenis_bayar = $this->input->post('jenis_pembayaran');
		//echo $grand_total.' '.$biaya_administrasi.' '.$biaya_materai.' '.$biaya_daftar.' '.$jasa_perawat;
		$vtot_peserta = $grand_total+$biaya_administrasi+0+$biaya_daftar+$jasa_perawat;
		...
		if($pasien[0]['carabayar']=='UMUM' || $pasienold[0]['cara_bayar']=='UMUM' ){
			$list_pa_pasien = $this->rimpasien->get_list_all_pa_pasien($no_ipd);
			$list_lab_pasien = $this->rimpasien->get_list_all_lab_pasien($no_ipd);
		...
		$grand_total = (double) $grand_total + (double) $pasien[0]['biaya_administrasi'] + (double) $pasien[0]['jasaperawat'] + 6000 + (double) $pasien[0]['biaya_daftar'];
  ...
  //       	exit;
		...
		if($pasien[0]['carabayar']=='BPJS'){
			$pasienold=$this->rimtindakan->get_old_pasien($pasien[0]['noregasal']);
			if($pasienold[0]['cara_bayar']=='BPJS'){
				if(($poli_irj_dokter_kelompok) or ($poli_irj_dokter)){
				$konten= $konten.'
				<table class="table-isi" border="0">
					<tr>
		...
		// $konten = $this->string_data_pasien($pasien,$grand_total,$penerima,'').$konten;
			...
			$konten0 = $this->string_data_pasien_noheader($pasien,$grand_total,"",'').$konten0;
...
//echo $konten;
	...
	private function string_data_pasien_noheader($pasien,$grand_total,$penerima,$jenis_pembayaran){
		// $konten="";
		// $format_tanggal = date("j F Y", strtotime($pasien[0]['tgl_masuk']));
		// $konten = $konten."
		// <table>
    ...
    private function string_table_resep($list_resep, $cara_bayar, $list_resep_subtotal){
        $konten = "";
        //<table border="1">
        $konten= $konten.'
		<table class="table-isi" border="0">
        ...
        $subtotal = 0; $total_bayar = 0;
        foreach ($list_resep as $r) {

            if($cara_bayar != 'UMUM'){
                $vtot = $r['vtot'];
            }else{
                $vtot = (int) (100 * ceil($r['vtot'] / 100));
            }

            $subtotal += $vtot;
            $vtots = number_format($vtot,0) ;
            $konten = $konten. "
			<tr>
			<td colspan=\"5\" >".$r['nama_obat']."</td>
			<td align=\"center\">".$r['Satuan_obat']."</td>
			<td align=\"center\">".$r['qty']."</td>
			<td align=\"right\">".$vtots."</td>
		</tr>
			";
        }

        if($cara_bayar == 'BPJS') {
            foreach ($list_resep_subtotal as $row) {
                $subtotal = (int) (1000 * ceil($row['subtotal'] / 1000));
	...
	private function string_table_resep_detail($list_resep, $cara_bayar, $list_resep_subtotal){
		$konten = "";
		//<table border="1">
		$konten= $konten.'
		<table border="1">
  ...
  //       	exit;
		...
		//resep
		if(($list_resep)){
			$result = $this->string_table_resep_detail($list_resep, $pasien[0]['carabayar'], $list_resep_subtotal);
			$grand_total = $grand_total + $result['subtotal'];
			// $konten = $konten."
			// <tr>
	...
	public function view($no_ipd=''){
		...
		//$this->load->view('iri/rivmenu', $data);
		//$this->load->view('iri/rivstatus');
		$this->load->view('iri/list_status_view',$data);
		//$this->load->view('iri/rivfooter');
	}
}
