...
class Rickwitansi extends Secure_area {
	...
	public function index(){
		...
		$data['list_pasien'] = $this->rimpasien->select_pasien_iri_pulang_belum_cetak_kwitansi();
	...
	public function detail_kwitansi($no_ipd=''){
		$data['title']='Kwitansi Rawat Inap Pasien';
		$data['reservasi']='';
		$data['daftar']='';
		$data['pasien']='';
		$data['mutasi']='';
		...
		if($pasien[0]['carabayar']=='UMUM'){
			...
			$data['list_tindakan_ird'] = $this->rimpasien->get_list_tindakan_ird_pasien($pasien[0]['noregasal']);
			if($pasienold[0]['cetak_kwitansi']==0 && $pasienold[0]['cara_bayar']=='BPJS'){
				...
				$data['list_pa_pasien'] = $this->rimpasien->get_list_all_pa_pasien($no_ipd);
				$data['list_lab_pasien'] = $this->rimpasien->get_list_all_lab_pasien($no_ipd);
				$data['list_radiologi'] = $this->rimpasien->get_list_all_radiologi_pasien($no_ipd);				
			}
		}

		$obat=0;
		if(($data['list_resep'])){
				$result = $this->string_table_resep_simple($data['list_resep']);
				$obat = $result['subtotal'];
		}

		$data['total'] = $this->get_grandtotal_all_noobat($no_ipd) + $obat;
		$data['grand_total'] = $this->get_grandtotal_all_noobat($no_ipd);
		//echo $data['grand_total'];
		//buat pembulatan ke atas 500
		$uang = $data['grand_total'] + $obat;
		$ratusan = substr($uang, -3);
		 ...
		 $biaya_administrasi= (double) $data['grand_total'] * 0.07;
		...
		$jasa_perawat=0;//$jasaperawat;
		$biaya_daftar=0;
		// $pasien_cetak_kw_rj = $this->rimpasien->get_list_cetak_kwitansi_rj($pasien[0]['noregasal']);
		// if($pasien_cetak_kw_rj[0]['cetak_kwitansi']==1){
		// 	$biaya_daftar=0;
		// }
		// else{
		...
		$data['biaya_daftar'] = $biaya_daftar;
		 $data['grand_total_pembulatan'] = $data['grand_total_pembulatan'] + $data['biaya_administrasi'] + $jasa_perawat + $biaya_daftar +0;// 6000;
		 $data['grand_total'] = $data['grand_total'] + $data['biaya_administrasi']+ $obat + $jasa_perawat + $biaya_daftar + 0;//6000;
		 //echo $data['grand_total'];
		//tambah cara bayar
	...
	public function get_grandtotal_all_noobat($no_ipd){
		$pasien = $this->rimtindakan->get_pasien_by_no_ipd($no_ipd);
		$pasienold = $this->rimtindakan->get_old_pasien($pasien[0]['noregasal']);		
