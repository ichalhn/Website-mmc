...
class Mctindakan_2 extends Secure_area {
	public function __construct(){
		parent::__construct();

		$this->load->model('master/mmtindakan_2','',TRUE);
	}

	public function index(){
		$data['title'] = 'Master Tindakan';

		$data['tindakan']=$this->mmtindakan_2->get_all_tindakan()->result();
		$data['kelas']=$this->mmtindakan_2->get_all_kelas()->result();
		$data['kel_tindakan']=$this->mmtindakan_2->get_all_kel_tindakan()->result();
		$this->load->view('master/mvtindakan_2',$data);
		//print_r($data);
	}
	...
	public function insert_tindakan(){
		...
		//insert to jenis_tindakan
		$this->mmtindakan_2->insert_jenis_tindakan($jt);
		// echo json_encode($jt);
		...
		//get kelas
		$kelas=$this->mmtindakan_2->get_all_kelas()->result();
		foreach ($kelas as $row) {
			...
			$this->mmtindakan_2->insert_tarif_tindakan($data);
		...
		redirect('master/mctindakan_2','refresh');
		//print_r($jt);
	}

	public function get_data_edit_tindakan(){
		$idtindakan=$this->input->post('idtindakan');
		$datajson=$this->mmtindakan_2->get_data_tindakan($idtindakan)->result();
	    echo json_encode($datajson);
	}

	public function delete_tindakan($idtindakan=''){
		$datajson=$this->mmtindakan_2->delete_tindakan($idtindakan);
		$success = 	'<div class="content-header">
					<div class="box box-default">
						<div class="alert alert-success alert-dismissable">
							<button class="close" aria-hidden="true" data-dismiss="alert" type="button">×</button>
	...
	public function edit_tindakan(){
		...
		$this->mmtindakan_2->edit_jenis_tindakan($idtindakan, $jt);
		...
		foreach ($kelas as $row) {
			...
			$id_tarif_tindakan=$this->mmtindakan_2->return_tarif($idtindakan, $row->kelas)->row()->id_tarif_tindakan;
			$this->mmtindakan_2->edit_tarif_tindakan($id_tarif_tindakan, $data);
		}
	...
	public function export_excel(){
		...
		$tindakan=$this->mmtindakan_2->get_all_tindakan()->result();
