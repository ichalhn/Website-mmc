...
function update_status_pemeriksaan_optic(no_ipd){
	var r = confirm("Anda yakin ingin menambah pemeriksaan Patologi Anatomi ?");
	if (r == true) {
	   $.ajax({
		    type:'POST',
						...
						<table class="table-sm table-striped" style="font-size:15">
						  <tbody>
							...
							<tr>
								<th></th>
								<td>&nbsp; </td>
								<td><?php if($data_pasien[0]['carabayar']=='DIJAMIN'){echo $data_pasien[0]['nmkontraktor'];}?></td>
							</tr>
							<tr>
								<th></th>
								<td>&nbsp;</td>
							...
							<tr>
								<th></th>
								<td>&nbsp;</td>
								<td><br><a href="<?php echo base_url();?>iri/rictindakan/patient_note/<?php echo $data_pasien[0]['no_ipd'];?>"><input type="button" class="btn btn-primary btn-sm" value="Catatan Perkembangan Pasien"></a></td>
						...
						<div class="form-group row">
							...
							<div class="col-sm-12">
								...
								<?php 
									if($row->status_rad>='1'){
										if($row->rad=='0'){
										echo '<input type="checkbox" id="radCheckbox"  class="flat-red" name="radCheckbox"  value="1"> Diagnostik | '.$row->status_rad.' Done';
										} else {
											echo '<input type="checkbox" id="radCheckbox"  class="flat-red" name="radCheckbox" checked="checked" value="1" disabled> Diagnostik';
											echo ' | <a href="'.base_url('rad/radcdaftar/pemeriksaan_rad').'/'.$data_pasien[0]['no_ipd'].'" target="_blank">Progress</a>';
										}
									}
									else { 
										if($row->rad=='0'){
										echo '<input type="checkbox" id="radCheckbox"  class="flat-red" name="radCheckbox"  value="1"> Diagnostik';
										} else {
											echo '<input type="checkbox" id="radCheckbox"  class="flat-red" name="radCheckbox" checked="checked" value="1" disabled> Diagnostik';
											echo ' | <a href="'.base_url('rad/radcdaftar/pemeriksaan_rad').'/'.$data_pasien[0]['no_ipd'].'" target="_blank">Progress</a>';
										}
								...
								<label class="checkbox-inline no_indent">
								<?php 
									// if($row->status_optik>='1'){
										//if($row->optik=='0'){
									//	echo '<input type="checkbox" id="optikCheckbox"  class="flat-red" name="optikCheckbox"  value="1"> Optik | '.$row->status_optik.' Done';
									//	} else {
										//	echo '<input type="checkbox" id="optikCheckbox"  class="flat-red" name="optikCheckbox" checked="checked" value="1" disabled> Optik';
									//		echo '';
								//		}
								//	}
								//	else { 
									//	if($row->optik=='0'){
							//			echo '<input type="checkbox" id="optikCheckbox"  class="flat-red" name="optikCheckbox"  value="1"> Optik';
								//		} else {
							//				echo '<input type="checkbox" id="optikCheckbox"  class="flat-red" name="optikCheckbox" checked="checked" value="1" disabled> Optik';
								//			echo ' ';
									//	}
