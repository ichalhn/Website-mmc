...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    table_iri = $('#table-iri').DataTable({ 
      ...
      "ajax": {
        "url": "<?php echo site_url('ina-cbg/iri/get_klaim')?>",
        "type": "POST",
        "data": function (data) {
      ...
      "columnDefs": [{ 
        ...
        "render": function ( data, type, row ) {
          switch(data) {
              case '1' : return '<p class="text-center"><button type="button" class="btn btn-sm btn-primary text-bold" style="margin-right:3px;" onclick="show_set_klaim(\''+row[4]+'\')"><small>Set</small></button><button type="button" class="btn btn-sm btn-success text-bold" style="margin-right:3px;" onclick="show_pasien(\''+row[4]+'\')"><small>Edit</small></button><a class="delete_patient btn btn-sm btn-danger text-bold" href="'+row[1]+'"><small>Hapus</small></a></p>'; break;
              default  : return '<center><button type="button" class="btn btn-sm btn-primary text-bold" onclick="add_claim(\''+row[4]+'\')"><i class="fa fa-id-card-o"></i> Buat Klaim</button></center>';
          }
        },
        "targets": 5 // column index 
      },{ "width": "14%", "targets": 1 },{ "width": "5%", "targets": 0 }],
  ...
  function add_claim(no_sep){
    $.ajax({
        type: "GET",
        url: "<?php echo base_url().'ina-cbg/iri/show_pasien/'; ?>"+no_sep,
        dataType: "JSON",
  ...
  function show_set_klaim(no_sep){
    $.ajax({
        type: "GET",
        url: "<?php echo base_url().'ina-cbg/iri/show_setklaim/'; ?>"+no_sep,
        dataType: "json",
        success: function(data){
          var diags = [];
          diags.push(data.utama);
          $.each(data.tambahan, function(i, item) {
            diags.push(item);
          })
          var diagnosa = diags.join('#');
  ...
  function insert_klaim(){
    document.getElementById("submit-add-klaim").innerHTML = '<i class="fa fa-spinner fa-spin"></i> Loading...';
    $.ajax({
        type: "POST",
        url: "<?php echo base_url().'ina-cbg/iri/add_claim'; ?>",
        dataType: "JSON",
  ...
  function update_patient(){
    document.getElementById("submit-edit-klaim").innerHTML = '<i class="fa fa-spinner fa-spin"></i> Loading...';
    $.ajax({
        type: "POST",
        url: "<?php echo base_url().'ina-cbg/iri/update_patient'; ?>",
        dataType: "JSON",
  ...
  function delete_patient(nomor_rm){    
    $.ajax({
        type: "POST",
        url: "<?php echo base_url().'ina-cbg/iri/delete_patient'; ?>",
        dataType: "JSON",
  ...
  function set_klaim(){
    document.getElementById("submit-set-klaim").innerHTML = '<i class="fa fa-spinner fa-spin"></i> Loading...';
    $.ajax({
        type: "POST",
        url: "<?php echo base_url().'ina-cbg/iri/set_claim'; ?>",
        dataType: "JSON",
          ...
          <div class="box">
            ...
            <div class="box-body">
             <form id="form-filter" class="form-horizontal" name="form_filter">
              ...
              <table id="table-iri" class="table table-bordered table-hover" width="100%">
                <thead>
                <tr>
                  <th>No.</th>
                  <th>No. CM</th>
