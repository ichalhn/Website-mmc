...
class Mmobat extends CI_Model{
    ...
    function getAllCaraPakai($id = NULL){
        if($id != null) {
            $where = " WHERE id = ".$id;
        }else{
            $where = "";
        }
        return $this->db->query("SELECT * FROM master_cara_pakai $where ORDER BY cara_pakai ASC");
    }

    function edit_carapakai($id, $carapakai){
        return $this->db->query("UPDATE master_cara_pakai SET cara_pakai = '$carapakai' WHERE id = ".$id);
    }

    function insert_carapakai($data){
        $this->db->insert('master_cara_pakai', $data);
        return true;
    }
