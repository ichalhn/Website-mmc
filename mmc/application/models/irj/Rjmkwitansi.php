<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Rjmkwitansi extends CI_Model{
		...
		function getdata_tindakan_pasien($no_register){
			return $this->db->query("SELECT nmtindakan, no_register, vtot, bpjs FROM pelayanan_poli where no_register='$no_register'
				union SELECT 'OBAT' AS nmtindakan, no_register, vtot_obat as vtot, '0' as bpjs FROM daftar_ulang_irj where no_register='$no_register'
				union SELECT 'LABORATORIUM' AS nmtindakan, no_register, vtot_lab as vtot, '0' as bpjs FROM daftar_ulang_irj where no_register='$no_register'
				union SELECT 'RADIOLOGI' AS nmtindakan, no_register, vtot_rad as vtot, '0' as bpjs FROM daftar_ulang_irj where no_register='$no_register'
				union SELECT 'TINDAKAN OPERASI' AS nmtindakan, no_register, vtot_ok as vtot, '0' as bpjs FROM daftar_ulang_irj where no_register='$no_register'
				union SELECT 'TINDAKAN LAIN' AS nmtindakan, no_register, vtot_lain as vtot, '0' as bpjs FROM daftar_ulang_irj where no_register='$no_register'");
		}

		function getdata_unpaid_finish_tindakan_pasien($no_register){			
			return $this->db->query("SELECT *, 
			(select COUNT(bpjs) from pelayanan_poli where no_register='$no_register') as noncover 
			FROM pelayanan_poli 
		...
		function getdata_unpaid_tindakan_pasien($no_register){			
			return $this->db->query("SELECT * FROM pelayanan_poli where no_register='$no_register' and idtindakan LIKE '1B01%'
				order by xupdate desc");
		}
		...
		function get_vtot($no_register){
			return $this->db->query("SELECT vtot, vtot_ok, vtot_lab, vtot_rad, vtot_obat, vtot_pa, vtot_lain FROM daftar_ulang_irj WHERE no_register='$no_register'");
		}
		/////////////////////////////////////////////////////////////////////////////////
...
and A.no_resep=B.no_resep");
		...
		function edit_biaya_tambahan($no_register,$data){
			$this->db->where('no_register', $no_register);
			$this->db->update('daftar_ulang_irj', $data); 
			return true;
		}

		function getdata_cetak_test($no_register){
			return $this->db->query("SELECT 'OBAT' as Tindakan, A.no_register, A.tgl_kunjungan, A.vtot_obat, B.nmtindakan, A.qty, A.vtot FROM daftar_ulang_irj A, jenis_tindakan B where no_register='$no_register' and A.idtindakan=B.idtindakan");
		}
	}
?>
