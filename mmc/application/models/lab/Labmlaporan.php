	...
	class Labmlaporan extends CI_Model{
		...
		function get_data_kunj_today(){
			return $this->db->query("SELECT b.no_cm, a.no_medrec, a.no_register, nama, a.tgl_kunjungan as tgl, count(1) as banyak 
				...
				AND left(a.tgl_kunjungan,10)  = left(now(),10) SELECT a.*, b.nama as nama FROM pendapatan_rad as a
					LEFT JOIN data_pasien as b
					ON a.no_medrec=b.no_cm
					WHERE LEFT(a.tgl_kunjungan , 10) >= '$awal'
					AND LEFT(a.tgl_kunjungan , 10) <= '$akhir'
					AND no_rad IS NOT NULL
					ORDER BY a.tgl_kunjungan, no_rad")
				GROUP BY a.no_register 
			UNION
					SELECT 'Pasien Luar' as no_cm, c.no_medrec, c.no_register, nama, c.tgl_kunjungan as tgl, count(1) as banyak 
		...
		function get_data_kunj_by_date($tgl){
			return $this->db->query("SELECT b.no_cm, a.no_medrec, a.no_register, nama, count(1) as banyak 
				...
				GROUP BY a.no_register 
				UNION
					SELECT 'Pasien Luar' as no_cm, c.no_medrec, c.no_register, nama, count(1) as banyak 
				FROM pemeriksaan_laboratorium c, pasien_luar d 
				WHERE c.no_register=d.no_register 
		...
		function get_data_keu_tind($awal, $akhir){
			return $this->db->query("SELECT a.*, b.nama as nama FROM pendapatan_lab as a
					...
					ORDER BY a.tgl_kunjungan, no_lab");
		}
