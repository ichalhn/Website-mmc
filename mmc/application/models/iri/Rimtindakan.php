...
class Rimtindakan extends CI_Model {
	...
	public function insert_tindakan_real($data){
		$querynoantrian="(SELECT IFNULL(CONCAT('T', LPAD (max(right(id_jns_layanan,5))+1 ,5,0) ),'T00001') 
	...
	public function select_maxno_tindakan(){
		$data=$this->db->query("select max(id_jns_layanan) as id_jns_layanan from pelayanan_iri");
		return $data->result_array();
	}
...
Limit 1");
	...
	public function get_list_tindakan_pasien_by_no_ipd_temp($no_ipd){
		...
		/*select *, (SELECT nmruang from ruang where idrg=a.idrg) as nmruang,a.xuser as user_input, (select nm_dokter from data_dokter where id_dokter=a.idoprtr) as nmdokter
			...
			order by tgl_layanan asc*/
		$data=$this->db->query("
			select a.*, c.*, d.nmruang, a.xuser as user_input, e.nm_dokter as nmdokter
			from pelayanan_iri_temp as a inner join data_pasien as b on a.nomederec = b.no_medrec
			inner join jenis_tindakan as c on a.id_tindakan = c.idtindakan
            inner join ruang as d ON d.idrg=a.idrg
            inner join data_dokter as e on e.id_dokter=a.idoprtr
			where a.no_ipd = '$no_ipd' 		
			order by tgl_layanan asc");
		return $data->result_array();
	}
...
/*SELECT * FROM pelayanan_iri, data_dokter, jenis_tindakan
	...
	public function get_list_tindakan_pasien_by_no_ipd($no_ipd){
		$data=$this->db->query("
			select *, IF(c.idkel_tind=0,'',(select nama_kel from kel_tind where idkel_tind=c.idkel_tind)) as nama_kel, (select nm_dokter from data_dokter where id_dokter=a.idoprtr) as nm_dokter, (select nm_dokter from data_dokter where id_dokter=a.idoprtr) as nmdokter, d.nmruang 
...
where
	...
	public function get_list_alat_pasien_by_no_ipd($no_ipd,$case){
		...
		if($case!=0){
			$query='and kel_tind.idkel_tind IN (1,2,4,5,6,7,8,9,10,17)';
		}
		else{
...
where
	...
	public function get_pasien_by_no_ipd($no_ipd){
		$data=$this->db->query("
			select a.*, a.jasa_perawat as jasaperawat, b.*, c.vtot as vtot_ruang, c.*,d.*,e.*, f.nm_diagnosa as nm_diagmasuk, g.nmkontraktor			
			...
			left join kontraktor as g on g.id_kontraktor=a.id_kontraktor
			where a.no_ipd = '$no_ipd' 
	...
	public function insert_tindakan_temp_new($data){

		return $this->db->insert('pelayanan_iri_temp', $data);
		// if(!$this->db->insert('pelayanan_iri_temp', $data)){
		// 	print_r($this->db->error());
		// 	exit; 
		// }else{
