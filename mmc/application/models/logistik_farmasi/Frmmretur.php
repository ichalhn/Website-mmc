<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Frmmretur extends CI_Model{
		...
		function get_data_retur_by_id($nofaktur){
			...
			return $this->db->query("SELECT p.*, r.qty AS quantity_retur, o.hargajual, SUM(g.qty) AS stok, o.nm_obat
				FROM po p
				left JOIN master_obat o ON o.`id_obat` = p.`item_id`
				left JOIN gudang_inventory g ON g.`id_obat` = p.`item_id` AND g.batch_no = p.batch_no
				left JOIN do d ON d.id_po = p.id_po
				LEFT JOIN retur r ON r.`no_faktur` = d.`no_faktur`
		...
		function get_laporan_retur($param1, $param2){
            return $this->db->query("SELECT r.`no_faktur`, r.`tgl_retur`, SUM(r.`qty`) AS qty_retur, o.`nm_obat`, o.`satuank`, g.`batch_no`, g.`expire_date`, s.`company_name`, o.`satuank`
                ...
                LEFT JOIN po p ON p.`no_faktur` = r.`no_faktur`
                LEFT JOIN header_po h ON h.`id_po` = p.`id_po`
                LEFT JOIN suppliers s ON s.`person_id` = h.`supplier_id`
