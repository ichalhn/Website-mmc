  ...
  class Frmmdistribusi extends CI_Model{
	...
	function get_amprah_detail_acc($data){
        return $this->db->query("SELECT a.id, a.id_amprah, a.id_obat, a.id_gudang, a.id_gudang_tujuan, a.satuank, a.qty_req, a.qty_acc, a.expire_date, a.batch_no, a.keterangan, a.user
			...
			AND a.qty_acc IS NOT NULL AND a.batch_no IS NOT NULL AND a.expire_date IS NOT NULL GROUP BY id_obat, qty_acc")->result();
    }
	...
	function get_obat_distribusi(){
		return $this->db->query("SELECT d.`id_obat`, d.`batch_no`, o.`nm_obat`
								FROM distribusi d
								INNER JOIN master_obat o ON o.`id_obat` = d.`id_obat`
								INNER JOIN amprah a ON a.`id_amprah` = d.`id_amprah`
								WHERE a.`minggu` != 0 AND qty_acc != 0
								GROUP BY o.`nm_obat`");
	}

	function get_nama_gudang($idamprah){
		return $this->db->query("SELECT a.`id_amprah`, a.`tgl_amprah`, g.`nama_gudang` AS asal, gg.`nama_gudang` AS tujuan
						FROM amprah a
						INNER JOIN master_gudang g ON g.`id_gudang` = a.`gd_asal`
						INNER JOIN master_gudang gg ON gg.`id_gudang` = a.`gd_dituju`
						WHERE a.`id_amprah` ='$idamprah'");
	}

      function get_data_distribusi_obat($idamprah){
          return $this->db->query("SELECT a.id, a.id_amprah, a.id_obat, a.id_gudang, a.id_gudang_tujuan, 
                a.satuank, a.qty_req, a.qty_acc, a.expire_date, a.batch_no, a.keterangan, a.user, o.`nm_obat`
                FROM distribusi a
                INNER JOIN master_obat o ON a.`id_obat` = o.`id_obat`
                WHERE a.id_amprah = '$idamprah'
                AND a.qty_acc IS NOT NULL AND a.batch_no IS NOT NULL AND a.expire_date IS NOT NULL");
      }
}
