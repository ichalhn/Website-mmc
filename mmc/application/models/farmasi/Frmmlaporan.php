...
SELECT 'Pasien Luar' as no_cm, c.no_register, nama, count(1) as banyak, sum(vtot) as vtot 
		...
		function get_data_keu_tind($awal, $akhir, $jenisbayar){
			return $this->db->query("SELECT*FROM pendapatan_farmasi
                    WHERE tgl_kunjungan >= '$awal' AND tgl_kunjungan <= '$akhir'
                    AND cara_bayar = '$jenisbayar' ORDER BY tgl_kunjungan , no_resep ASC");
		}
		...
		function find_no_racik($param1='' ,$param2='', $jenis){
            return $this->db->query("SELECT no_resep, sum(vtot) as subtotal FROM resep_pasien
                WHERE LEFT(tgl_kunjungan , 10) >= '$param1' AND LEFT(tgl_kunjungan , 10) <= '$param2'
                AND no_resep IS NOT NULL AND cara_bayar = '$jenis'
                GROUP BY no_resep");
        }

        function get_data_keu_tind_new($no_resep, $jenis){
            return $this->db->query("SELECT*FROM pendapatan_farmasi
                    WHERE no_resep = '$no_resep' AND cara_bayar = '$jenis'
                    ORDER BY tgl_kunjungan , no_resep ASC");
        }
	}
?>